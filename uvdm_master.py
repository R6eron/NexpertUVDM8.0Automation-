...
def init(self, state_path: Path = STATE_PATH, legacy_path: Path = LEGACY_IDENTITY_FILE):
...
class Journal:
def init(self, log_path: Path = EXECUTION_LOG, receipt_dir: Path = RECEIPT_DIR):
...
raise ValueError(
f"Requested  {intent.limit_value_usd:,.2f}"
)
...
class MasterConsole:
def init(self, identity_manager: IdentityManager, journal: Journal, risk: RiskEngine, router: Router):
...
print(
f"Portfolio:  {intent.limit_value_usd:,.2f})."
)
print(
