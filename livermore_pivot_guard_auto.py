...
STATE_PATH = os.path.expanduser('~/.xrpeasy_pivot_state')

print(f"ðŸš€ XRPEASY LIVERMORE PIVOT GUARD | {datetime.now().strftime('%H:%M GMT')}")
...
def write_pivot_state(state, follow_through, spring_condition, price, change):
    ...
    try:
        ...
        print(f'âš ï¸ STATE WRITE ERROR: {e}')
...
if price is not None:
    print(f'ðŸ“Š XRP/USDT: ${price:.6f} | 24h: {change:+.2f}% | STATE: {state} | FOLLOW_THRU: {follow_through} | SPRING: {spring_condition}')
else:
    print('âŒ INITIAL API ERROR - RETRYING')

write_pivot_state(state, follow_through, spring_condition, price, change)
print('ðŸ”„ UVDM LIVERMORE SENTINEL LIVE - Ctrl+C to exit')
...
while True:
    ...
    if price is not None:
        print(f"[{datetime.now().strftime('%H:%M')}] XRP: ${price:.6f} ({change:+.2f}%) â†’ {state} | FOLLOW_THRU: {follow_through} | SPRING: {spring_condition}")
