...
STATE_PATH = os.path.expanduser('~/.sgbeasy_pivot_state')

print(f"[SGBEASY] LIVERMORE PIVOT GUARD | {datetime.now().strftime('%H:%M GMT')}")
...
if price is not None:
    print(f'[DATA] SGB/USDT: ${price:.6f} | 24h: {change:+.2f}% | STATE: {state} | FOLLOW_THRU: {follow_through} | SPRING: {spring_condition}')
else:
...
while True:
    ...
    if price is not None:
        print(f"[{datetime.now().strftime('%H:%M')}] SGB: ${price:.6f} ({change:+.2f}%) -> {state} | FOLLOW_THRU: {follow_through} | SPRING: {spring_condition}")
