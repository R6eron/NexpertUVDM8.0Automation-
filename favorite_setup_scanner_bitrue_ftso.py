...
SESSION.headers.update({
    'User-Agent': 'UVDM8/1.0 (+Termux; favorite-setup-scanner)',
...
MEXC_INTERVAL_MAP = {'15m': '15m', '30m': '30m', '1h': '60m'}
...
def get_bitrue_klines(symbol, interval, limit=LIMIT):
    ...
    try:
        ...
        raise RuntimeError(f"Non-JSON kline response for {symbol} {interval}: {r.text[:120]!r}") from e
    if not isinstance(rows, list) or len(rows) == 0:
        raise RuntimeError(f"Empty/invalid kline payload for {symbol} {interval}: {rows!r}")
    return {
...
def get_mexc_klines(symbol, interval, limit=LIMIT):
    ...
    try:
        ...
        raise RuntimeError(f"Non-JSON MEXC response for {symbol} {interval}: {r.text[:120]!r}") from e
    if not isinstance(rows, list) or len(rows) == 0:
        raise RuntimeError(f"Empty/invalid MEXC payload for {symbol} {interval}: {rows!r}")
    return {
        'open': [float(x[1]) for x in rows],
        'high': [float(x[2]) for x in rows],
        'low': [float(x[3]) for x in rows],
...
def get_klines(symbol, interval, limit=LIMIT):
    try:
        return get_bitrue_klines(symbol, interval, limit)
    except Exception as e:
        print('KLINE_FALLBACK', symbol, interval, 'bitrue_failed', repr(e))
